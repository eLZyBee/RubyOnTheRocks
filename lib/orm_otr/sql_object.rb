...
class SQLObject
  def self.columns
    unless @instance_variables
      instance_variables = DBLink.execute2(<<-SQL)
  ...
  def insert
    ...
    DBLink.execute(<<-SQL, *attribute_values)
      INSERT INTO #{self.class.table_name} (#{col_names})
      VALUES (#{question_marks})
    SQL
    self.id = DBLink.last_insert_row_id
  end

  def update
    cols = self.class.columns[1..-1].map { |var| var.to_s + ' = ?'}
    col_sets = cols.join(', ')
    DBLink.execute(<<-SQL, *attribute_values.rotate)
